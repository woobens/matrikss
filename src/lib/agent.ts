...
export class TestAgent {
   ...
   * @param matriksIns hojs实例
   */
  constructor(method: SUPPORT_METHODS, path: string, key: string, sourceFile: SourceResult, matriksIns: any) {
  ...
  private saveExample() {
    ...
    if (this.options.takeExample) {
      this.options.matrikss.api.$apis.get(this.key)!.example({
  ...
  private output(raw = false, save = false) {
    this.options.matrikss.api.$apis.get(this.key)!.options.tested = true;
    return this.options.agent!.then((res) => {
      this.options.agentOutput = res.body;
      if (raw) return res;
      const formatOutputReverse = this.options.matrikss.api.formatOutputReverse || defaultFormatOutput;
