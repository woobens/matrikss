...
export default class IAPIDoc {
  ...
  constructor(matriksIns: MAtrikss<any>) {
    this.matrikss = matriksIns;
    const { info, groups, docsOptions } = this.matrikss.privateInfo;
    this.info = info;
    this.groups = groups;
  ...
  public buildDocData() {
    ...
    const data: IDocData = {
      ...
      errorManager: this.matrikss.errors,
      schema: this.matrikss.schema,
      typeManager: this.matrikss.type,
      group: this.groups,
    ...
    const formatOutput = this.matrikss.api.docOutputForamt || docOutputFormat;

    // types
    this.matrikss.type.forEach((item, key) => {
    ...
    for (const [k, schema] of this.matrikss.api.$apis.entries()) {
