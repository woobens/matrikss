...
import MAtrikss, { IApiOptionInfo } from "..";
...
export default class IAPITest {
  private matrikss: MAtrikss<any>;
  private info: IApiOptionInfo;
  private app: any;
  private testPath: string;
  private supertest?: any;

  constructor(matriksIns: MAtrikss<any>, path: string) {
    this.matrikss = matriksIns;
    const { info, app } = this.matrikss.privateInfo;
    this.info = info;
    this.app = app;
    this.testPath = path;
  ...
  public session(): ITestSession {
    ...
    const buildSession = (method: SUPPORT_METHODS) => {
      return (path: string) => {
        ...
        const a = new TestAgent(method, path, s.key, s.options.sourceFile, this.matrikss);
  ...
  private findApi(method: SUPPORT_METHODS, path: string) {
    ...
    // 检查path无变量情况
    if (this.matrikss.api.$apis.get(key)) {
      return this.matrikss.api.$apis.get(key);
    }
    // 检查path有变量情况
    for (const s of this.matrikss.api.$apis.values()) {
  ...
  private buildTest(method: SUPPORT_METHODS) {
    return (path: string) => {
      ...
      const a = new TestAgent(method, path, s.key, getCallerSourceLine(this.testPath), this.matrikss);
