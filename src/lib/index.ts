 ...
 */
export default class MAtrikss<T = DEFAULT_HANDLER> {
  ...
  public checkerLeiWeb<K>(ereat: MAtrikss<T>, schema: API): (ctx: K) => void {
  ...
  public checkerExpress<U, V, W>(ereat: MAtrikss<T>, schema: API): (req: U, res: V, next: W) => void {
   ...
   */
  public bindRouter(router: any, checker: (ctx: MAtrikss<T>, schema: API<T>) => T) {
    if (this.forceGroup) {
   ...
   */
  public bindRouterToApp(app: any, Router: any, checker: (ctx: MAtrikss<T>, schema: API<T>) => T) {
    if (!this.forceGroup) {
